...
namespace IngameScript {
  partial class Program {
    public class Container {
      public string DisplayName => _cargo.DisplayNameText;
      ...
      public int GetAffinity(MyInventoryItem item) {
        ...
        if (_isOutput) {
          ...
          if(_subtypes.Contains(item.GetItemSubype().ToLower())) {
            return hasItem + 8;
          }
      ...
      private bool _hasItem(MyInventoryItem item) => _cargo.GetInventory().FindItem(item.Type) != null;
    }
  }
}
