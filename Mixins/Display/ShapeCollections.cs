      ...
      public void Parse(string data) {
        ...
        foreach (string line in data.Split(SEPLN).Select(l => l.Trim())) {
          ...
          if (line.StartsWith("=")) {
            ...
            this.sprites.Add(line.Substring(1).Trim(), sprts);
          } else {
            ...
            sprts.Add(this.toSprt(vals[0].Trim(), color, this.parseVector2(vals[2], count, "position"), this.parseVector2(vals[3], count, "size"), rotation, isCentered));
          }
        }
      }

      void throwIf(bool shouldThrow, int line, string msg) {
        if (shouldThrow) {
          throw new InvalidOperationException($"Error at line {line}: {msg}"); 
        }
      }

      Color parseColor(string s, int line) {
        string[] cs = s.Split(SEP_VEC);
        if (cs.Count() == 1) {
          return this.scheme.GetColor(cs[0].Trim());
        } else if (cs.Count() == 3) {
          int r, g = 0, b = 0;
          bool res = int.TryParse(cs[0], out r) && int.TryParse(cs[1], out g) && int.TryParse(cs[2], out b);
          this.throwIf(!res, line, "could not parse rgb color");
          return new Color(r, g, b);
        } else {
          this.throwIf(true, line, "not enough or too many values for the color");
          return new Color();
        }
      }

      Vector2 parseVector2(string s, int line, string name) {
        string[] ent = s.Split(SEP_VEC);
        if (ent.Count() != 2) {
          this.throwIf(true, line, $"not enough or too many values for the {name}");
        }
        float x, y = 0;
        bool res = float.TryParse(ent[0], out x) && float.TryParse(ent[1], out y);
        this.throwIf(!res, line, $"could not parse {name}");
        return new Vector2(x, y);
      }

      MySprite toSprt(string s, Color col, Vector2 pos, Vector2 size, float rot, bool isCentered) {
