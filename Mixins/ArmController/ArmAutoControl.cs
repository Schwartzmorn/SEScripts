...
namespace IngameScript
{
  partial class Program
  {
    ...
    public class ArmAutoControl
    {
      ...
      public bool Control(List<ArmRotor> rotors, IMyShipController cont)
      {
        ...
        if (Target.Type == ArmPosType.Angle)
        {
          foreach (ArmRotor r in rotors)
          {
            ...
            {
              _initialDirection = Math.Sign(delta);
            }
            reachedDestination &= Math.Abs(delta) < 0.01 || _initialDirection != Math.Sign(delta);
          }
        }
        else
        {
          ...
          float speed = MathHelper.Clamp((float)delta * 20, -maxSpeed, 4);
          ...
          reachedDestination = Math.Abs(delta) < 0.01 || Math.Sign(delta) != _initialDirection;
        }
      ...
      /// <summary>
      /// returns a a vector o such that
      ///  - length 1
      ///  - orthogonal to refV
      ///  - in the same plane than v and refV
      ///  - v.o > 0
      /// </summary>
      /// <param name="v"></param>
